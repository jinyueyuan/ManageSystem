...
class CourseCatalog
{
    ...
    public function sqlQuery(){
        ...
        if(!empty($_POST['search']) && $_POST['search']=="搜索"){
            $key = $_POST['key'];
            $keySql = "SELECT * FROM course WHERE course_id LIKE '%$key%' OR cou_name LIKE '%$key%' OR credits LIKE '%$key%' LIMIT $this->limit";
            $this->catalogResult = mysql_query($keySql);
        }else{
            $catalogSql = "SELECT * FROM course WHERE grade_id=$grade_id LIMIT $this->limit";
    ...
    private function getTotal(){
        ...
        if(!empty($_POST['search']) && $_POST['search']=="搜索"){
            $key = $_POST['key'];
            $keySql = "SELECT * FROM course WHERE grade_id=$grade_id AND (course_id=$key OR cou_name LIKE '%$key%')";
            $catalogResult = mysql_query($keySql);
        }else{
            $catalogSql = "SELECT * FROM course WHERE grade_id=$grade_id";
